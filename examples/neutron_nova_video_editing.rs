...
impl<E: Engine> SpartanCircuit<E> for FreivaldsConvCircuit<E::Scalar> {
  ...
  fn synthesize<CS: ConstraintSystem<E::Scalar>>(
    ...
    // 7. Polynomial interpolation verification.
...
fn main() {
  ...
  // Use a random circuit of the right shape for key derivation.
  let shape_circuit =
  ...
  let t0 = Instant::now();
  // prove takes prep by reference and returns the SNARK directly (not a tuple)
  let snark =
